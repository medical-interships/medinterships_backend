...
exports.updateApplicationStatus = async (req, res) => {
  try {
    ...
    const io = getIO()
    io.to(`user-${application.studentId}`).emit(socketEvents.APPLICATION_UPDATED, {
      id: application.id,
    ...
    await NotificationService.notifyApplicationUpdate(application.studentId, application.id, status)

    if (status === "acceptee") {
      await NotificationService.sendToUser(application.studentId, {
        type: "success",
        title: "Candidature acceptée",
        message: `Félicitations! Votre candidature pour "${internship.title}" a été acceptée.`,
      ...
      await NotificationService.sendToUser(application.studentId, {
        ...
        message: `Votre candidature pour "${internship.title}" a été refusée.`,
        relatedEntityType: "Application",
        relatedEntityId: application.id,
      })
    }
